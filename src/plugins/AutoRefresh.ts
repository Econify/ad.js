...
class AutoRefresh extends GenericPlugin {
  ...
  public afterRender() {
    this.dispatchEvent(this.ad.id, LOG_LEVELS.INFO, 'AutoRefresh Plugin', 'Adding monitoring for ad.');
    this.startMonitoringViewability();
    this.dispatchEvent(this.ad.id, LOG_LEVELS.INFO, 'AutoRefresh Plugin', 'Ad viewability is being monitored.');
  }

  public beforeClear() {
    ScrollMonitor.unsubscribe(this.ad.el.id);
    this.dispatchEvent(this.ad.id, LOG_LEVELS.INFO, 'AutoRefresh Plugin', 'Ad viewability monitor has been removed.');
  }
  ...
  private markAsOutOfView = () => {
    ...
    this.dispatchEvent(this.ad.id, LOG_LEVELS.INFO, 'AutoRefresh Plugin', 'Ad is no longer in view.');
